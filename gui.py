...
class MenuFrame(tk.Frame):
    def __init__(self, parent, city):
        ...
        self.accident_label = tk.Label(self, text='Accidents: {}'.format(self.city.accidents), font=(None, 20))
        ...
        self.add_button = tk.Button(self, text='Add Car', command=self.city.add_random_agent)
        self.add_button.pack(pady=5)

        self.add_boost_button = tk.Button(self, text='Add {} Cars'.format(int(self.city.vertical_roads_count+self.city.horizontal_roads_count)), command=self.city.add_multiple_agents)
        self.add_boost_button.pack(pady=5)

        self.add_super_boost_button = tk.Button(self, text='Add {} Cars'.format(int(self.city.vertical_roads_count+self.city.horizontal_roads_count)*5), command=self.city.add_times_multiple_agents)
...
class CityFrame(tk.Frame):
    ...
    def update_accidents(self, delta_t):
        ...
        for each in self.city.accidents_list[:]:
            ...
            x, y = self.get_drawing_position((c1.x + c2.x)/2, (c1.y + c2.y)/2)
